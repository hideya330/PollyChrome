...
- **翻訳字幕の表示**: 外国語の読み上げ時に、画面下部に日本語訳を映画の字幕のように表示します。（長文の改行やスクロールにも対応）
- **読み上げ箇所のハイライト**: 読み上げ中の原文を画面下部に表示し、現在読まれている単語を音声と同期してハイライト表示します。（カラオケ字幕風）
- **辞書・翻訳ツールチップ**: わからない文章や単語を選択して「📖 辞書/翻訳」ボタンを押すと、その場で文章の翻訳と、文章内に含まれる各英単語の意味をリストアップして表示します。（単語の活用形を考慮した賢い抽出と、大容量のストップワード除外機能を搭載）
- **便利なUIと操作性**:
  - ドラッグ時に出現するフローティングボタン（読み上げ / 辞書・翻訳 / 設定）
  - 右クリックメニューからの実行
  - カスタマイズ可能なショートカットキー（読み上げ、停止、再再生）
  - 読み上げ中のコントロールパネル（停止・再再生・mp3ファイルの保存・予想コストの表示）
- **細かなカスタマイズ**: オプション画面から、言語ごとの音声タイプ、読み上げ速度、コスト計算用の為替レート、各種機能（音声・字幕・ハイライト・ボタン）のON/OFF、辞書機能で除外する単語（ストップワード）を自由に設定可能。
- **安心の安全装置**: 1回あたり5,000文字の上限チェックと、AWS側での利用回数・頻度制限（Usage Plan）の二重の防衛ラインにより、クラウド破産を確実に防ぎます。
...
flowchart TB
    ...
    subgraph Local_PC ["Local PC / Google Chrome Browser"]
        ...
        subgraph Content_Script ["Web Page Context (content.js)"]
            ...
            FloatingBtn["2. Floating Buttons<br>🔊 読み上げ / 📖 辞書・翻訳 / ⚙️ 設定"]
        ...
        subgraph Injected_UI ["Injected UI (via chrome.scripting)"]
            AudioPlayer["HTML5 Audio Player<br>Base64 MP3再生 / 再生速度・ハイライト同期"]
            Subtitle["Subtitle Panel<br>原文ハイライト表示 / 翻訳字幕表示"]
            CtrlPanel["Control Panel<br>⏹ 停止 / 🔄 再生 / 💾 保存 / ✖ 閉じる / コスト表示"]
        ...
        subgraph Extension_Background ["Extension Service Worker (background.js)"]
            ...
            MsgReceiver -- 'speak_text' --> FetchSpeak["Fetch API (Speak Mode)<br>※5000文字制限チェック"]
        ...
        subgraph Extension_Options ["Options Page (options.html/js)"]
            SettingsUI["Settings UI<br>- 機能ON/OFF<br>- 言語別音声 / 速度<br>- 為替レート / ストップワード"]
    ...
    subgraph Internet ["Internet"]
        HTTPS_REQ["HTTPS POST Request<br>Header: x-api-key<br>Body: text, voice_type, enable_*, stop_words, usd_rate..."]
        HTTPS_RES["HTTPS Response (JSON)<br>audio(Base64), speech_marks, translated_text, word_meanings, estimated_cost"]
    end
    ...
    subgraph AWS_Cloud ["AWS Cloud (Serverless Architecture)"]
        ...
        APIGW["Amazon API Gateway<br>- REST API エンドポイント<br>- Usage Plan (APIキー・頻度/回数制限)<br>- CORS設定 (OPTIONS)"]
...
4. 読み上げ中に画面右下に表示されるコントロールパネルから、再生の停止・再再生、および **「💾 保存」** ボタンによる音声のmp3ダウンロードが可能です。パネルには実行時の予想コストも表示されます。
5. 拡張機能のアイコンをクリックするか右クリックして **「オプション」** を開くと、言語ごとの声の種類の変更、速度の調整、コスト計算用の為替レート、各種機能のON/OFF、辞書機能で翻訳を除外する単語の設定（デフォルトリストの復元含む）が可能です。
...
**【無料枠超過後のシミュレーション（※1USD=150円換算）】**
- 1,000文字の英語記事を「翻訳字幕あり」で読み上げた場合：1回あたり **約2.8円**
- 約2,800文字の技術ドキュメントを「翻訳字幕あり＋辞書機能」でフル活用した場合：1回あたり **約8.4円**（※「音声再生のみ」なら **約1.7円**）
